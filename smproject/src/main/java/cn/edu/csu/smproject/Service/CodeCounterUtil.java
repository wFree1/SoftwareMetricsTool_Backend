...
public class CodeCounterUtil {
     ...
     * 代码行数统计
     */
    public  ArrayList<Code> countCode(MultipartFile[] files) {
        for (MultipartFile f : files) {
            if (f.getOriginalFilename().matches(".*\\.java$")){ // 匹配java格式的文件
                count(f);
     ...
     * @param f
     */
    private  void count(MultipartFile f) {
        //设置一个code
        Code code = new Code();
        code.setFileName(f.getOriginalFilename());
